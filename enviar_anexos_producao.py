...
def iniciar_processo_de_envio(caminho_xml, pasta_pdfs, relatorio_path='relatorio_envio_completo.txt'):
    """
    Função principal para orquestrar a análise do XML, busca de PDFs e envio.
    Remove a necessidade de interação manual e caminhos fixos.

    Args:
        caminho_xml (str): Caminho completo para o arquivo XML do lote.
        pasta_pdfs (str): Caminho para a pasta contendo os PDFs das guias.
        relatorio_path (str): Caminho onde o relatório de texto será salvo.

    Returns:
        dict: Um dicionário com o resumo do processamento.
    """
    print("="*80)
    ...
    LOGIN = "LAB0186"
    SENHA = "91a2ab8fbdd7884f7e32fd19694712a0"  # Senha em MD5
    REGISTRO_ANS = "005711"
    INTERVALO_ENVIOS = 2
    ...
    if not pacientes:
        ...
        return {'sucesso': False, 'mensagem': 'Nenhum paciente encontrado no XML.'}
    ...
    if pdfs_encontrados == 0:
        ...
        return {'sucesso': False, 'mensagem': 'Nenhum PDF correspondente foi encontrado.'}
    ...
    processador.gerar_relatorio(resultados, relatorio_path)
    ...
    print(f"📊 Total processado: {len(resultados)}, ✅ Sucessos: {sucessos}, ❌ Falhas: {falhas}")
    print(f"📄 Relatório completo salvo em: {relatorio_path}")

    taxa_sucesso = (sucessos / len(resultados) * 100) if len(resultados) > 0 else 0

    return {
        'sucesso': True,
        'mensagem': f'Processo finalizado. {sucessos} de {len(resultados)} envios com sucesso ({taxa_sucesso:.1f}%).',
